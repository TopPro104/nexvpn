...
- **Geo files**: in `<data>/geo/<hash of geosite+geoip URLs>/`, shared by every profile with the same URLs (custom rules without a profile use the Loyalsoldier defaults); unused folders are deleted, old per-profile folders are migrated at startup. Xray reads them via `XRAY_LOCATION_ASSET`; sing-box gets per-code source rule-sets built by `core/geo.rs`. Unknown codes are dropped (Xray won't start with them). Refreshed on new `LastUpdated`/URL or after 7 days
